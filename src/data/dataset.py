...
class Method1KariesDataset(Dataset):
    """Loads tooth-level crops (images and masks) generated by YOLO bbox export."""

    def __init__(self, crop_root, split, size=(256, 256), transform=None):
        ...
        metadata_path = os.path.join(crop_root, split, "metadata.csv")
        ...
        with open(metadata_path, "r") as f:
            ...
            for row in reader:
                # Column names from export_segmentation_crops_from_bboxes.py
                self.metadata.append({
                    "image": row["crop_image"],
                    "mask": row["crop_mask"]
                })
    ...
    def __getitem__(self, i):
        ...
        # Resize to fixed size for batching and memory predictability
        if self.size:
