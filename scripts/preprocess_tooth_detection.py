...
def infer_latest_dataset_dir() -> pathlib.Path:
    candidates = [p for p in RAW_DIR.glob("tooth-p8snt-*") if p.is_dir()]
    candidates.extend([p for p in RAW_DIR.glob("Tooth-*") if p.is_dir()])
    if not candidates:
        raise FileNotFoundError(
            "No tooth detection dataset directory found (expected tooth-p8snt-* or Tooth-* under data/raw)."
        )
