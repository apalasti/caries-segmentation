...
def main() -> None:
    ...
    def get_base_name(p_name: str) -> str:
        name = p_name
        if ".rf." in name:
        ...
        if img_dir.exists():
            for p in img_dir.iterdir():
                if p.suffix.lower() in VALID_EXTS:
                    ...
                    # Keep one uniquely, prioritize non-augmented
                    if base not in all_images_map:
                        all_images_map[base] = p
                    else:
                        existing = all_images_map[base]
                        if "_aug" not in p.name and "_aug" in existing.name:
                            all_images_map[base] = p

    all_images = list(all_images_map.values())
    print(f"Total unique base images after deduplicating augmentations: {len(all_images)}")
